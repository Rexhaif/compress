...
mod tests {
    ...
    #[test]
    fn reader_writer_output_is_stock_bzip2_compatible_when_available() {
        if !command_exists("bzip2") {
            return;
        }

        let input = b"streaming compatibility smoke\nstreaming compatibility smoke\n";
        let mut encoded = Vec::new();
        encode_reader_to_writer(input.as_slice(), &mut encoded, &options()).unwrap();
        let path = temp_path("reader-writer", "bz2");
        std::fs::write(&path, encoded).unwrap();
